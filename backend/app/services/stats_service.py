...
class StatsService:
                ...
                connections.append(
                    {
                        ...
                        "type": (
                            str(conn.type.name) if hasattr(conn.type, "name") else str(conn.type)
                        ),
                        "local_address": (
    ...
    async def get_process_list(
        self, limit: int = 20, order_by: str = "cpu"
    ) -> List[Dict[str, Any]]:
        processes = []
        try:
                    ...
                    processes.append(
                        {
                            ...
                            "cmdline": (
                                " ".join(pinfo.get("cmdline", [])) if pinfo.get("cmdline") else None
                            ),
    ...
    async def get_container_resource_stats(
        self, container_id: str, formatted: bool = True
    ) -> Dict[str, Any]:
    ...
    async def get_container_processes(self, container_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            processes = await docker_service._run_in_executor(container.top, ps_args="aux")
    ...
    async def collect_and_store_container_stats(
        self, container_id: str
    ) -> Optional[ContainerStats]:
        docker_service = DockerService(self.db)
    ...
    async def get_system_stats_history(
        ...
        query = (
            select(SystemStats)
            .where(SystemStats.timestamp >= start_time)
    ...
    def _aggregate_system_stats(
        ...
        for stat in stats:
            interval_timestamp = (
                int(stat.timestamp.timestamp() / interval_seconds) * interval_seconds
            )
            ...
            result.append(
                {
                    "timestamp": timestamp,
                    "cpu_usage_avg": (
                        sum(values["cpu_usage"]) / len(values["cpu_usage"])
                        if values["cpu_usage"]
                        else 0
                    ),
                    ...
                    "memory_usage_avg": (
                        sum(values["memory_usage"]) / len(values["memory_usage"])
                        if values["memory_usage"]
                        else 0
                    ),
                    "memory_usage_max": (
                        max(values["memory_usage"]) if values["memory_usage"] else 0
                    ),
                    "memory_usage_min": (
                        min(values["memory_usage"]) if values["memory_usage"] else 0
                    ),
    ...
    async def get_top_consumers(
        ...
        query = select(ContainerStats).where(ContainerStats.timestamp >= start_time)
        ...
        for container_id, container_data in container_stats.items():
            ...
            avg_cpu = (
                sum(s.cpu_usage for s in container_data) / len(container_data)
                if container_data
                else 0
            )
            max_cpu = max(s.cpu_usage for s in container_data) if container_data else 0
            avg_memory = (
                sum(s.memory_usage for s in container_data) / len(container_data)
                if container_data
                else 0
            )
            max_memory = max(s.memory_usage for s in container_data) if container_data else 0
            total_network = (
                sum(s.network_rx + s.network_tx for s in container_data) if container_data else 0
            )
    ...
    async def compare_containers(
        ...
        for container_id in container_ids:
            container_stats = await self.get_container_stats_history(
                container_id, period, aggregate=True
            )
    ...
    async def prune_old_stats(self, retention_days: int = 30) -> Dict[str, Any]:
        ...
        system_result = await self.db.execute(
            select(func.count()).select_from(SystemStats).where(SystemStats.timestamp < cutoff_date)
        )
        system_deleted = system_result.scalar() or 0

        await self.db.execute(
            SystemStats.__table__.delete().where(SystemStats.timestamp < cutoff_date)
        )

        container_result = await self.db.execute(
            select(func.count())
            .select_from(ContainerStats)
            .where(ContainerStats.timestamp < cutoff_date)
        )
        container_deleted = container_result.scalar() or 0

        await self.db.execute(
            ContainerStats.__table__.delete().where(ContainerStats.timestamp < cutoff_date)
        )
    ...
    async def get_container_group_stats(self, group_id: int) -> Dict[str, Any]:
        ...
        for container in containers:
            container_info = next(
                (c for c in container_list if c.get("id") == container.container_id), None
            )
            if container_info:
        ...
        return {
            ...
            "containers": [
                {"id": c.container_id, "name": c.name, "status": c.status} for c in containers
            ],
        }
